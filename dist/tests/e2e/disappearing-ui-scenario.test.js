...
describe('E2E Acceptance Scenario — Injected UI Disappearance Forensic Investigation', () => {
    it('should capture injected UI lifecycle, parent subtree replacement, and prove root cause via MCP', async () => {
        ...
        const timelineData = JSON.parse((timelineRes?.result).content[0].text);
        ...
        const diagnosis = JSON.parse((diagnosisRes?.result).content[0].text);
        ...
        expect(diagnosis.precedingEvents.some((e) => e.type === 'NETWORK_RESPONSE_COMPLETE')).toBe(true);
        expect(diagnosis.followingEvents.some((e) => e.type === 'RUNTIME_ERROR')).toBe(true);
    });
});
//# sourceMappingURL=disappearing-ui-scenario.test.js.map