...
export class LifecycleTracer {
    static traceElement(target, events, initialSnapshot) {
        ...
        for (const evt of sortedEvents) {
            ...
            if (evt.type === 'DOM_MUTATION_ATTR') {
                ...
                if (payload.nodeId === targetId) {
                    ...
                    if (attr === 'class')
                        stage = 'CLASS_MODIFIED';
                    if (attr === 'style')
                        stage = 'STYLE_MODIFIED';
                    entries.push({
                        timestamp: ts,
                        sequence: seq,
                        wallClockTime: wall,
        ...
        const windowMs = 500;
        const correlatedDiagnostics = sortedEvents.filter((e) => (e.category === 'ERROR' || e.category === 'CONSOLE') &&
...
//# sourceMappingURL=lifecycle-tracer.js.map