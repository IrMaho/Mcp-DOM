...
class ForensicDashboardApp {
    ...
    renderTimelineTracks() {
        ...
        for (const evt of this.events) {
            ...
            if (evt.category === 'DOM')
                domLane.appendChild(dot);
            else if (evt.category === 'USER')
                userLane.appendChild(dot);
            else if (evt.category === 'ERROR' || evt.category === 'CONSOLE')
                errorLane.appendChild(dot);
            else if (evt.category === 'NETWORK')
                netLane.appendChild(dot);
        }
    ...
    `;
        document.getElementById('btn-quick-trace')?.addEventListener('click', () => {
            ...
            const tabForensics = document.querySelector('[data-tab="forensics"]');
            tabForensics?.click();
            document.getElementById('forensic-target-input').value = String(nodeId);
            this.runForensics();
        });
        document.getElementById('btn-quick-forensics')?.addEventListener('click', () => {
            const tabForensics = document.querySelector('[data-tab="forensics"]');
            tabForensics?.click();
            document.getElementById('forensic-target-input').value = selector;
            this.runForensics();
        });
    }
    runDiff() {
        if (!this.reconstructor)
            return;
        const t1 = parseFloat(document.getElementById('diff-t1-input').value) || 0;
        const t2 = parseFloat(document.getElementById('diff-t2-input').value) || 100;
    ...
    diffAroundCurrentTime() {
        if (!this.reconstructor || !this.timeController)
            return;
        const curr = this.timeController.getCurrentTime();
        const t1 = Math.max(0, curr - 50);
        const t2 = curr + 50;
        document.getElementById('diff-t1-input').value = t1.toFixed(0);
        document.getElementById('diff-t2-input').value = t2.toFixed(0);
        const s1 = this.reconstructor.getStateAt({ timestamp: t1 });
        const s2 = this.reconstructor.getStateAt({ timestamp: t2 });
        const diff = DOMDiffEngine.diff(s1, s2);
        this.renderDiffResult(diff);
    ...
    runForensics() {
        const input = document.getElementById('forensic-target-input').value.trim();
        if (!input)
            return;
    ...
    renderDiagnosticsTab() {
        ...
        list.innerHTML = diagnostics
            .map((d) => {
            const isErr = d.category === 'ERROR' || d.payload?.level === 'error';
            const msg = d.payload?.formattedMessage || d.payload?.message || '';
            const stack = d.payload?.stack || d.payload?.stackTrace || '';
            return `
    ...
    renderNetworkTab() {
        ...
        list.innerHTML = network
            ...
            const method = n.payload?.method || 'GET';
            const url = n.payload?.url || '';
            const dur = n.payload?.durationMs;
    ...
    async exportSession() {
        ...
        const annotations = await this.storage.getAnnotations(this.currentSession.id);
        const bundle = SessionSerializer.exportBundle(this.currentSession, initialSnapshot, this.events, checkpoints, annotations);
