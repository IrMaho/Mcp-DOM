...
export class MCPToolsHandler {
    ...
    async handleTraceElement(args) {
        ...
        const trace = LifecycleTracer.traceElement({ nodeId: args.nodeId, selector: args.selector }, events, initialSnapshot || undefined);
        if (!trace) {
            return {
                isError: true,
    ...
    async handleGetDiagnostics(args) {
        ...
        const filtered = events.filter((e) => {
            if (e.category !== 'CONSOLE' && e.category !== 'ERROR')
                return false;
            if (typeof args.fromTimestamp === 'number' && e.timestamp < args.fromTimestamp)
                return false;
            if (typeof args.toTimestamp === 'number' && e.timestamp > args.toTimestamp)
                return false;
            if (args.level && args.level !== 'all') {
                const level = e.payload?.level;
                if (args.level === 'error' && e.category !== 'ERROR' && level !== 'error')
                    return false;
                if (args.level !== 'error' && level !== args.level)
                    return false;
            }
            return true;
        });
