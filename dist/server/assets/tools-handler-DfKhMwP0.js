		...
		for await (const line of rl) {
			...
			if (filter) {
				...
				if (filter.searchQuery) {
					...
					if (!JSON.stringify(e.payload || {}).toLowerCase().includes(query) && !e.type.toLowerCase().includes(query)) continue;
				}
			}
			matchedCount++;
...
var VirtualTreeBuilder = class VirtualTreeBuilder {
	...
	markSubtreeDetached(nodeId) {
		...
		if (node.children && node.children.length > 0) for (const childId of node.children) this.markSubtreeDetached(childId);
	}
	applyMove(payload) {
	...
	toHTML(nodeId = this.rootId, indent = 0) {
		...
		if (node.isShadowRoot || node.nodeType === VirtualDOMNodeType.DOCUMENT_FRAGMENT_NODE) return `${spacing}<template shadowrootmode="${node.shadowMode || "open"}">\n${(node.children || []).map((c) => this.toHTML(c, indent + 1)).join("\n")}\n${spacing}</template>`;
...
var VirtualQueryEngine = class {
	...
	static getElementById(id, nodes) {
		for (const node of Object.values(nodes)) if (node.nodeType === VirtualDOMNodeType.ELEMENT_NODE && !node.isDetached && node.attributes && node.attributes["id"] === id && this.isNodeConnected(node, nodes)) return node;
		return null;
	}
...
//#endregion
export { SessionSerializer as n, FileStorageProvider as r, MCPToolsHandler as t };
