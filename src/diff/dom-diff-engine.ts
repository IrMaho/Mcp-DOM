...
export class DOMDiffEngine {
  public static diff(s1: DOMSnapshot, s2: DOMSnapshot): DOMDiffResult {
    const addedNodes: AddedNodeDiff[] = [];
    const removedNodes: RemovedNodeDiff[] = [];
    const movedNodes: MovedNodeDiff[] = [];
    const changedAttributes: AttributeChangeDiff[] = [];
    const changedClasses: ClassChangeDiff[] = [];
    const changedStyles: StyleChangeDiff[] = [];
    const changedText: TextChangeDiff[] = [];

    const nodes1 = s1.nodes;
    const nodes2 = s2.nodes;

    const ids1 = new Set(
      Object.values(nodes1)
        .filter((n) => !n.isDetached)
        .map((n) => n.id)
    );
    const ids2 = new Set(
      Object.values(nodes2)
        .filter((n) => !n.isDetached)
        .map((n) => n.id)
    );
    ...
    const hasVisibilityChanges =
      changedClasses.some((c) =>
        [...c.addedClasses, ...c.removedClasses].some((cls) =>
          /\b(hidden|hide|d-none|invisible|visible|show)\b/i.test(cls)
        )
      ) ||
      changedStyles.some((s) =>
        ['display', 'visibility', 'opacity'].includes(s.propertyName.toLowerCase())
      );
  ...
  private static renderNodeSnippet(node: VirtualDOMNode): string {
