...
export class VirtualQueryEngine {
  public static matches(node: VirtualDOMNode, selector: string): boolean {
  ...
  public static querySelectorAll(
    ...
    const results: VirtualDOMNode[] = [];
    const root = nodes[rootId];
    if (!root) return results;

    const queue: LogicalNodeId[] = [...(root.children || [])];
  ...
  private static matchesCompound(node: VirtualDOMNode, selector: string): boolean {
    // Basic single element selector matching: tag#id.class[attr=val]
    return this.matchesSimple(node, selector);
  }

  private static matchesSimple(node: VirtualDOMNode, selector: string): boolean {
