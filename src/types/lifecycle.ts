...
export type DisappearanceMechanism =
  | 'DIRECT_NODE_REMOVAL'
  | 'PARENT_SUBTREE_REPLACED'
  | 'PARENT_NODE_REMOVED'
  | 'STYLE_DISPLAY_NONE'
  ...
  | 'STYLE_OPACITY_ZERO'
  | 'CLASS_TRIGGERED_HIDDEN'
  | 'DIMENSIONS_COLLAPSED'
  | 'UNKNOWN';
...
export interface DisappearingElementReport {
  ...
  precedingEvents: BaseEvent[]; // events occurring right before disappearance (e.g. 500ms window)
  followingEvents: BaseEvent[]; // events occurring right after disappearance
